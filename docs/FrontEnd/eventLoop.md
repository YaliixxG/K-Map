...
1. 初始化代码（包括一些特殊代码，设置定时器，绑定监听，网络请求 ajax）
...
#### 模型解析  
<font color="#893DEW">首先JS引擎执行代码（ECMAScript），将代码的变量、事件等等放入堆或者栈中，这一步进行初始化。初始化过程中，将回调函数按顺序放入回调队列。代码通过浏览器API进行调用执行，浏览器调用中发现此事件中有回调函数需要执行时，会将事件发回给JS引擎。JS引擎收到发回的事件，在初始化所有事件完成后，再去回调队列中按顺序执行对应的回调函数。</font>
