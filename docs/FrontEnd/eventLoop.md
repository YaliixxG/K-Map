...
分析：如果是多线程的话，JS 在运行到定时器时，应该会开一个  线程来运行，到耗时的循环结构时，应该也会再开一个线程单独运行。但是返回的结果说明，JS 是将定时器放到了最后运行，先运行了其他的部分，包括循环，所以由此可见 JS 为单线程。
...
1. 初始化代码（包括一些特殊代码，设置定时器，绑定监听，网络请求 ajax）-- 同步代码
2. 回调代码（后面在某个时刻才可能执行回调代码）-- 异步代码

先执行初始化代码，初始化代码执行完毕，才执行回调代码。

### 为什么 JS 为单线程，而不改为多线程呢？

JS 的用途主要是用于与用户的互动，以及操作 DOM。如果是多线程会带来复杂的同步问题。例如在一个线程中进行删除 DOM，却在另一个线程中要求修改这个 DOM，这样就会造成很复杂的情况。所以用途决定了 JS 为单线程。

### 浏览器内核

支持浏览器运行的最核心程序，相当于人的大脑，计算机的 CPU。
...
<h4>浏览器通过调用一个一个的模块，来执行JS发来的一个一个请求。</h4>  

### 事件循环模型  
![事件循环模型](../.vuepress/public/imgs/eventLoopModel.png)  

### 模型解析  
1. 首先JS引擎执行代码（ECMAScript），将代码的变量、事件等等放入堆或者栈中，这一步进行初始化。初始化过程中，将回调函数按顺序放入回调队列。  
2. 初始化后，JS引擎分发任务给浏览器，通过浏览器API对应模块进行解析，调用，执行。  
3. 浏览器调用中发现此事件中有回调函数需要执行时，会将事件发回给JS引擎。  
4. JS引擎收到发回的事件，在“初始化类型代码”完成后，再去回调队列中按顺序执行对应的回调函数。
